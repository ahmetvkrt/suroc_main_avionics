...
{
    printf("[HAL] Step A: Starting sh2_i2c_hal_open()\r\n");

    if (isOpen)
    {
        printf("[HAL] ERROR: Already open!\r\n");
        return SH2_ERR;
    }

    printf("[HAL] Step B: Setting up I2C...\r\n");
    i2cBusState = BUS_INIT;
    i2cAddr = ADDR_SH2_0 << 1;
    isOpen = true;

    printf("[HAL] Step C: Asserting reset (rstn LOW)...\r\n");
    // Hold in reset, not for DFU
    rstn(false);
    inReset = true;  // will change back to false when INTN serviced

    printf("[HAL] Step D: Enabling interrupts...\r\n");
    enableInts();

    printf("[HAL] Step E: Delaying for RESET_DELAY_US (%d us)...\r\n", RESET_DELAY_US);
    // Delay for RESET_DELAY_US to ensure reset takes effect
    delay_us(RESET_DELAY_US);
    
    printf("[HAL] Step F: Transitioning to IDLE state...\r\n");
    ...
    printf("[HAL] Step G: Setting BOOTN HIGH...\r\n");
    // Deassert BOOT, don't go into bootloader
    bootn(true);

    printf("[HAL] Step H: De-asserting reset (rstn HIGH)...\r\n");
    // Deassert reset
    rstn(true);
    printf("Before Delay us \r\n");

    // ← BURAYA EKLEYİN:
    delay_us(10000);  // 100ms bekle
    GPIO_PinState intState = HAL_GPIO_ReadPin(INTN_PORT, INTN_PIN);
    printf("[HAL] INT Pin State: %d (0=LOW, 1=HIGH)\r\n", intState);
    printf("[HAL] Expected: 0 (LOW) if BNO085 is working\r\n");

    printf("[HAL] Step I: Waiting for INTN (timeout: %d us)...\r\n", START_DELAY_US);
    printf("[HAL] inReset = %d\r\n", inReset);

    // Wait for INTN to be asserted
    reset_delay_us(START_DELAY_US);

    printf("[HAL] Step J: After reset_delay_us, inReset = %d\r\n", inReset);

    if (inReset) {
        printf("[HAL] ERROR: INTN was NOT received! Sensor not responding.\r\n");
        printf("[HAL] Check INT pin connection (PB4).\r\n");
        return SH2_ERR;
    }

    printf("[HAL] Step K: INTN received! sh2_i2c_hal_open() SUCCESS\r\n");
    return SH2_OK;
}
